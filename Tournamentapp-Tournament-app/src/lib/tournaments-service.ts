...
export const deleteTournament = async (id: string) => {
    ...
    try {
        await runTransaction(firestore, async (transaction) => {
            ...
            // Step 1: Refund entry fee if it's greater than zero
            if (entryFee > 0 && participants.length > 0) {
                const userIds = new Set<string>();
                participants.forEach(team => {
                    team.members?.forEach(member => {
                        if (member.uid) userIds.add(member.uid);
                    });
                });
                ...
                for (const userId of userIds) {
                    ...
                    if (userDoc.exists()) {
                        // Refund the balance
                        transaction.update(userDocRef, { balance: increment(entryFee) });
                        ...
                        transaction.set(newTransactionRef, {
                            userId: userId,
                            amount: entryFee,
                            type: 'deposit', // Treat refund as a deposit/credit
            ...
            // Step 2: Delete the tournament document
