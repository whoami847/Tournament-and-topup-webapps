...
export async function POST(req: NextRequest) {
    try {
        ...
        const { amount, userId, tournamentId, description } = body;
        ...
        if (!tournamentId) {
            return NextResponse.json({ message: 'Tournament ID is missing.' }, { status: 400 });
        }
        ...
        // Fetch tournament to ensure it exists
        ...
        const gatewaysRef = collection(firestore, 'gateways');
        ...
        const newOrder: Order = {
            ...
            description: description || `Tournament Entry Fee - ${tournamentData.name}`,
        ...
        await setDoc(doc(firestore, "orders", transaction_id), newOrder);
        ...
        if (response.ok && result.payment_url) {
            ...
            await updateDoc(doc(firestore, "orders", transaction_id), { status: 'FAILED' });
