  ...
  return (
    <>
      ...
      <nav className="hidden md:block fixed top-4 left-4 z-50">
        <div className="bg-card/90 backdrop-blur-sm rounded-2xl border border-border shadow-lg p-2">
          <div className="flex flex-col gap-2">
            {navItems.map((item) => {
              const isActive = getIsActive(item.href);
              return (
                <Link key={item.label} href={item.href} className={cn(
                  "flex items-center gap-3 px-4 py-3 rounded-xl transition-all group hover:bg-accent",
                  isActive ? 'bg-primary text-primary-foreground shadow-lg' : 'text-muted-foreground hover:text-foreground'
                )}>
